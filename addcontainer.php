<?php
  session_start();
  if (!isset($_SESSION['username'])) {
      header('location:login.php');
  }
?>
<!DOCTYPE html>
<html lang="en" dir="ltr">
<head>
  <meta charset="utf-8">
  <meta name="viewport" content="width=device-width, initial-scale=1">
  <title>Maersk Line</title>
  <link rel="icon" href="img/logo.png">
  <link rel="stylesheet" type="text/css" href="css/style.css">
  <link rel="stylesheet" href="fontawesome/css/all.css">

  <!---------- RANDOM CONTAINER NUMBER GENERATOR --------->
  <script language="javascript" type="text/javascript">
    function randomString() {
    	var chars = "0123456789ABCDEFGHIJKLMNOPQRSTUVWXTZ";
    	var string_length = 10;
    	var randomstring = '';
    	for (var i=0; i<string_length; i++) {
    		var rnum = Math.floor(Math.random() * chars.length);
    		randomstring += chars.substring(rnum,rnum+1);
    	}
    	document.containerform.containerno.value = randomstring;
    }
  </script>
  </head>
  <body>
    <header>
      <div class="main">
        <div class="logo">
          <img src="img/logo.png" height="30" alt="">
          <h2><b>MAERSK</b> LINE</h2>
        </div>
        <div class="bars">
          <span id="two" onclick="closeNav() ">&#9776;</span>
          <span id="one" onclick="openNav()">&#9776;</span>
        </div>
        <a href="logout.php"><button type="button" class="logout-btn" name="button"><i class="fas fa-sign-out-alt"></i> logout</button></a>
      </div>
    </header>

    <section>
      <div id="mySidenav" class="sidebar">
        <h2>Welcome</h2>
        <ul>
          <li><a href="index.php" class="nav-btn"><i class="fas fa-tachometer-alt"></i> Dashboard</a></li>
          <li>
            <a href="#" class="nav-btn dropdown-btn"><i class="fas fa-building"></i> Transportation Office</a>
            <ul class="dropdown-container">
              <li><a href="addoffice.php" onclick="addClient()"><i class="fas fa-user-plus"></i> Add New</a></li>
              <li><a href="viewoffice.php"><i class="fas fa-search"></i> View All</a></li>
            </ul>
          </li>
          <li>
            <a href="#" class="nav-btn dropdown-btn active"><i class="fas fa-ship"></i> Container Management</a>
            <ul class="dropdown-container">
              <li><a href="addcontainer.php" onclick="addClient()"><i class="fas fa-user-plus"></i> Add New</a></li>
              <li><a href="viewcontainer.php"><i class="fas fa-search"></i> View All</a></li>
            </ul>
          </li>
          <li>
            <a href="#" class="nav-btn dropdown-btn"><i class="fas fa-route"></i> Shipping Route</a>
            <ul class="dropdown-container">
              <li><a href="addroute.php" onclick="addClient()"><i class="fas fa-user-plus"></i> Add New</a></li>
              <li><a href="viewroute.php"><i class="fas fa-search"></i> View All</a></li>
            </ul>
          </li>
          <li><a href="customer.php" class="nav-btn"><i class="fas fa-users"></i> Customer Details</a></li>
          <li><a href="customerrequest.php" class="nav-btn"><i class="fas fa-envelope-open-text"></i> Customer Request</a></li>
          <li><a href="bookings.php" class="nav-btn"><i class="fas fa-book"></i> Booking Details</a></li>
          <li><a href="notification.php" class="nav-btn"><i class="fas fa-bell"></i> Notification</a></li>
        </ul>
      </div>
    </section>

    <section>
      <div id="content" class="content">
        <div class="content-header">
            <h1>Add Container</h1>
            <ul class="breadcrumb">
                <li><a href="index.php"><i class="fas fa-bus"></i> Home</a></li>
                <li><a href="#">Container Management</a></li>
                <li>Add Contaienr</li>
            </ul>
        </div>

        <div class="content-form">
          <form action="addcontainerprocess.php" method="post" name="containerform">
            <div class="form-header">
              <h1>Container Details</h1>
            </div>
            <hr>
            <div class="form-box">
                <div class="row">
                  <div class="col-50" style="width: 100%; display: none;">
                      <input class="input" type="text" name="adminname" id="adminname" value="<?php echo $_SESSION['username']; ?>" placeholder="Username" required />
                  </div>
                  <div class="col-40">
                      <label>Container Number</label>
                      <input class="input" type="text" name="containerno" id="conrainerno" value="" required />
                  </div>
                  <div class="col-10">
                      <button type="button" name="generate" onClick="randomString();" id="generateCN">Generate</button>
                  </div>

                  <div class="col-50" style="padding-left: 10px;">
                      <label>Tare Weight</label>
                      <input class="input" type="number" name="tareweight" id="tareweight" placeholder="Kilogram" required/>
                  </div>
                  <div class="col-50">
                      <label>Payload Capacity</label>
                      <input class="input" type="number" name="payloadcapacity" id="payloadcapacity" placeholder="Kilogram" required/>
                  </div>
                  <div class="col-50" style="padding-left: 10px;">
                      <label>Cubic Capacity</label>
                      <input class="input" type="number" name="cubiccapacity" id="cubiccapacity" placeholder="Cubic Metre" required />
                  </div>
                  <div class="col-50">
                      <label>Internal Length</label>
                      <input class="input" type="number" name="internallength" id="internallength" placeholder="Metre" required/>
                  </div>
                  <div class="col-50" style="padding-left: 10px;">
                      <label>Internal Width</label>
                      <input class="input" type="number" name="internalwidth" id="internalwidth" placeholder="Metre" required />
                  </div>
                  <div class="col-50">
                      <label>Internal Height</label>
                      <input class="input" type="number" name="internalheight" id="internalheight" placeholder="Metre" required/>
                  </div>
                  <div class="col-50" style="padding-left: 10px;">
                      <label>Container Location</label>
                      <input class="input" type="text" name="containerlocation" id="containerlocation" placeholder="Container Location" autocomplete="off" required />
                  </div>
                  <div class="col-50">
                      <button type="submit" name="submitContainer">Submit</button>
                  </div>
                </div>
            </div>
          </form>
        </div>

      </div>
    </section>

  </body>
</html>
